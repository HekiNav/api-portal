...
import { Application, doServer, FormErrors } from "@/lib/definitions"
import { Add } from "@nine-thirty-five/material-symbols-react/sharp"
...
export default function EditApplication({ applications }: { applications: Application[] }) {
            ...
            <Modal open={visible} className="bg-white items-start w-120! max-h-8/10 overflow-y-scroll" cardTitle={`Editing ${name || "New application"}`}>
                <div className="px-4 flex flex-col w-full h-full">
                    ...
                    <span id="visibility" className="w-full flex gap-2 justify-center mt-2">
                        ...
                        <Button disabled={!success} onPress={() => !a ? toast.promise(doServer(new Promise((res) => res({success: false, message: "no func"}))), { loading: "Creating application", success: "Created application!", error: ({ message }) => `Could not create application: ${message}` }).then(() => {
                            router.refresh()
                            setVisible(false)
