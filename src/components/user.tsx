...
export default function UserUI({ user, isCurrentUser = false }: { user: User, isCurrentUser?: boolean }) {
    ...
    return (
        <div className="flex flex-col p-4 pb-0 font-sans">
            <div className="flex flex-row bg-blue-600 rounded-xl p-4 text-2xl font-mono text-white items-center">
            ...
            <div className="py-1">
                ...
                Last seen {Math.abs(Date.now() - (user?.lastSeen || 0)) > 1000 * 3600 * 24 * 10 ? dayjs((user?.lastSeen || 0) - 1000 * 3600 * 24 * 8).fromNow() : "recently"}
            </div>
            <h1 className="font-medium text-xl text-blue-600">Maps</h1>
            {user.maps?.length ? (
                            ...
                            <Card className="w-40!" small cardTitle={(<span className="font-bold">{m.name}</span>)}>
                                <span className={`mt-2 text-sm 
                                    ${
                                        ...
                                        m.visibility == MapVisibility.PUBLIC ? "bg-blue-600" :
                                        "bg-gray-300"
                                    } 
            ...
            <h1 className="font-medium text-xl text-blue-600 mt-4 mb-1">Friends</h1>
            {user.friends?.length ? (
                <div className="flex flex-row flex-wrap gap-4">{
                    ...user.friends.filter(f => f.state != FriendState.REJECTED).map((f, i) => (
                        <Link key={i} href={`/user/${(f.user1id == user.id ? f.user2 : f.user1)?.id}/`}>
                            <Card className="w-40!" small cardTitle={(<span className="font-bold">{(f.user1id == user.id ? f.user2 : f.user1)?.name}</span>)}>
                                <span className={`mt-2 text-sm ${f.state == FriendState.ACCEPTED ? "bg-blue-600" : "bg-yellow-600"} rounded px-1 h-min ml-1`}>{f.state}</span>
